# 开发路线图（截至 session 013）

## 一、最近修改总结

### Session 013（本次）：多 agent 并行（天界/分身/平衡/洞天v4/龙宫）
...
2. ~~**洞天系统**~~（session 009 洞天 v1 + v2 种植 + v3 聚灵阵 + session 013 v4 扩张经营：
   扩张碑 X 买灵田 6→12 块、阵眼 X 升级聚灵阵 0→2 级；design/dongtian.md）——
   后续可做：灵泉/丹房等新设施、洞天内小怪/采集点
3. ~~**技能/法术系统**~~（session 010 已落地 SkillSystem 管线：主动 13/被动 6 + 8 槽装配 + FX 9 种）——后续：组合技/派生
4. **法宝系统完善**（session 011 已落地 ArtifactSystem v1：本命+次要槽/温养/祭出）——
   次要法宝物品补全、槽位 UI（飞升前 3/后 6）；渡劫之地「只带本命法宝」规则未生效（v1 未禁用装备加成）
