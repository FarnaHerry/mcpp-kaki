...
6. **再迭代（v2 定案）**：「全屏的时候还是分辨率只有最低比例，我们直接原生支持各种分辨率即可，
   ...
   480 基准比例档静默自动匹配显示比例。
7. **v3（本轮）**：「现在所有状态都能调节分辨率，不要有遗留的东西了」
...
- **分辨率行 = 原生分辨率档**（常规游戏语义）：1280×720 / 1600×900 / **1920×1080（默认）** /
  2560×1440 / 3120×2080 / 3840×2160，cfg `res_idx` 持久化。**全窗口模式可调**（无灰显遗留）：
  - **窗口 / 无边框全屏**：档位=窗口尺寸（`window_set_size`，设后仍可自由拉伸）。
  - **独占全屏**：档位=显示模式（`window_set_size` 原生分辨率，影响渲染精度）。
- **内部渲染比例静默自动匹配**（无配置行）：content_scale_size 仍为 480 基准 5 档
  （16:9/16:10/3:2/4:3/21:9 单边延伸视野），`_sync_auto_aspect()` 按显示比例选最近档
  （独占全屏取所选分辨率比例、其余取窗口实际比例，0.001 滞回防抖），逐帧同步。
...
- test_settings_display 四写（41 断言，v3 语义）：6 行布局/旧行全删/窗口模式 3 档循环/
  分辨率行=原生档且**全窗口模式可调**（窗口档 root.size=所选分辨率、无边框档可切档非「=桌面」、
  独占档可切档）/无灰显文案（「自由拉伸」「=桌面」全清）/**内部比例自动匹配**
  （3120×2080→480×320、窗口拖 21:9→630×270）/cfg 旧键全消失。
