...
- **分辨率行 = 原生分辨率档**（常规游戏语义）：1280×720 / 1600×900 / **1920×1080（默认）** /
  2560×1440 / 3120×2080 / 3840×2160，cfg `res_idx` 持久化。
  - **独占全屏**：行可选，档位=显示模式（`window_set_size` 原生分辨率，影响渲染精度）。
  - **无边框全屏 / 窗口**：行灰显（无边框=桌面分辨率、窗口=用户自由拉伸），只展示不可调。
...
- **窗口大小游戏一概不碰**：窗口档用户自由拉伸；`_apply_window_geometry`/`_pending_geometry`
  全部删除——除独占全屏显示模式外没有任何代码 set 窗口尺寸。
...
- test_settings_display 三写（30 断言，v2 语义）：6 行布局/旧行全删/窗口模式 3 档循环/
  分辨率行=原生档（默认 1920×1080，窗口「自由拉伸」+无边框「=桌面」灰显文案）/独占全屏 ←/→ 切档
  （cfg res_idx）/**内部比例自动匹配**（3120×2080→480×320、窗口拖 3:2→480×320、21:9→630×270）/
  cfg 旧键全消失。
