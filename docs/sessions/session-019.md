...
- **设置页 6 行**：主音量 / 语言 / 窗口模式（3 档）/ **分辨率** / 保存 / 退出。
- **分辨率行 = 游戏自己的内部分辨率**（content_scale_size，和普通游戏的「分辨率」选项同一语义）：
  `480×270[16:9]` / `480×300[16:10]` / `480×320[3:2]` / `480×360[4:3]` / `630×270[21:9]` 5 档循环，
  cfg `res_idx` 持久化。
- **窗口大小游戏一概不碰**：窗口档用户自由拉伸、全屏=屏幕；`_apply_window_geometry`/`_pending_geometry`/
  `_sync_auto_aspect` 全部删除——没有任何代码再 set 窗口尺寸。
- 缩放固定整数倍；`aspect="keep"` 比例不符引擎居中黑边。
...
- test_settings_display 重写（25 断言）：6 行布局/旧行全删/档位格式 axb[16:9]/窗口模式 3 档循环/
  分辨率 5 档循环（content_scale_size+cfg res_idx）/**窗口改尺寸内部分辨率不变（无自动匹配）**/cfg 旧键全消失。
