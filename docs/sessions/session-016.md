...
**双轴扩展（用户三次反馈终修）**：expand+integer 只能处理「单轴超出整数倍」——16:9 整数倍窗口时
  ...
  终修：`_apply_content_scale(w,h)` 把 **content_scale_size 设为 窗口/整数scale**——视口逻辑尺寸=窗口比例，
  scale=N 整数（像素整齐不花屏），视口 ≥ 480×270（内容完整），宽高超出整数倍的余量由视口扩展显示更多世界
  （16:9 整数倍窗口→viewport 仍 480×270 完全兼容）。全屏档 `_apply_display` 置 pending，`_process`
  检测窗口尺寸连续帧稳定后对齐 content_scale 到屏幕。真机验证：全屏 3120×2080 → viewport 520×347、
  scale_size 520×347、四角采样背景色(17,17,17)非纯黑 → 双轴吃满无黑边。
