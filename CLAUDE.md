...
**UI 文本池 i18n（session 024 首轮）**：`LOC()` 查 `data/locale_en.json`（367→738 键，EN 翻译含页签/提示行/键位标签/境界·物品·敌人·能力·buff·法宝·洲数据名全套）；静态表**渲染期**包裹、`language_changed` 回调重取即时重译；**红线规范**：InputMap action 名/ConfigFile 段键/存档规范值（图鉴备注已修为存中文原文+显示 LOC、`boss_dead:` flag、pd/cd/data 键）一律裸字符串——此前 62+21 处 action 名误包 LOC 已解包（存档读写不对称 8 处同修）；**已知遗留**：物品 desc 47 条缺 EN 表项、GDScript spots 动态拼串 EN 下仍中文（后续轮）。
