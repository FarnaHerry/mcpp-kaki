...
- **Display**: 渲染 480×270（16:9 基准，按显示比例自动匹配 16:10/3:2/4:3/21:9 单边延伸视野）→ 窗口，canvas_items stretch + aspect=keep + 整数倍缩放；**渲染分辨率与窗口大小解耦**（普通游戏语义）；设置页分辨率行=原生分辨率（全窗口模式可调：窗口/无边框全屏=窗口尺寸、独占全屏=显示模式）
