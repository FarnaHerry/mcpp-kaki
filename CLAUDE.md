...
- **Display**: 渲染 480×270（16:9 基准，按显示比例自动匹配 16:10/3:2/4:3/21:9 单边延伸视野）→ 窗口，canvas_items stretch + aspect=keep + 整数倍缩放；**渲染分辨率与窗口大小解耦**（普通游戏语义）；独占全屏可选原生显示模式（设置页分辨率行）
