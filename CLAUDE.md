...
- **地府/生死簿/勾魂** (`design/cultivation-realms.md` §五) — **SoulLedgerSystem** (`src/core/soul_ledger_system.*`) 独立生死簿：簿上寿元=物种默认100 ≠ 实际寿元(随境界 凡人100/炼气150/筑基250/金丹500/元婴2000/化神5000…)，信息差=勾魂错抓；出身/原身/划名标记(免死一次)，存档 pd["soul_ledger"]；HUD 寿元小标签「寿 簿上/实际」（实际>簿上绿）+ SignalBus `lifespan_changed/soul_protection_changed/ledger_inspect_requested`。**勾魂使**：濒死(HP<20%)刷黑/白无常（Enemy `is_soul_reaper`+no_drops，realm≥玩家），反杀+30修为、被击杀→魂魄入地府。**死亡三分支** (`GameManager::on_player_died`)：免死(划名→原地满血复活，最高优先)/勾魂使击杀→入地府/正常→回检查点；地府内死亡→还阳。**地府场景** `scenes/continents/difu.tscn`（全场景切换，`enter_difu/huan_yang` 走 change_scene_to_file+旅行桥，**不走 request_scene_change** 防污染 `_respawn_scene`）：判官(`UnderworldInteractNode` mode=查簿, HUD overlay 显出身/原身/寿元/划名状态)+生死簿(mode=改簿划名→免死一次)+还阳出口；东胜神洲入口 `SceneGate`(scripts/gates/, ↑ 触发 gm_method)；互斥：地府内不再刷无常、子空间/机缘中不刷
