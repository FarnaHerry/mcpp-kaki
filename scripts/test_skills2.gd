...
func _process(delta) -> bool:
	...
	match _step:
		1:
			...
			# 旋风斩施放（装 A 槽）
			_check(sk.call("assign", 6, "xuan_feng_zhan"), "旋风斩装 A 槽")
			_check(sk.call("cast_slot", 6), "旋风斩施放 ok")
		2:
			_next = _t + 0.3
			# 升龙击：上跃速度
			var p = root.find_child("Player", true, false)
			var sk = p.call("get_skills")
			sk.call("assign", 7, "sheng_long_ji")
			_check(sk.call("cast_slot", 7), "升龙击施放 ok")
		...
		3:
			...
			sk.call("assign", 8, "lei_zhou_shu")
			var n0 = _count_projectiles()
			_check(sk.call("cast_slot", 8), "雷咒术施放 ok")
		...
		4:
			...
			sk.call("assign", 9, "tu_dun_shu")
			_check(sk.call("cast_slot", 9), "土盾术施放 ok")
		...
		5:
			...
			var cult = p.call("get_cultivation")
			cult.call("set_mana", 200.0)
			sk.call("assign", 8, "yu_jian_shu")
			var n0 = _count_projectiles()
			_check(sk.call("cast_slot", 8), "御剑术施放 ok")
		...
		6:
			...
			sk.call("assign", 4, "jin_gang_bu_huai")
			_check(sk.call("cast_slot", 4), "金刚不坏施放 ok")
			_cast_t = _t
		...
		8:
			...
			var s11 = sk.call("get_slot_info", 11)
			_check(String(s11.get("id")) == "tian_lei_yin", "天雷引自动装 H 槽（仙法）")
		10:
			if _fail == 0:
				print("[TEST] ALL PASS")
			else:
